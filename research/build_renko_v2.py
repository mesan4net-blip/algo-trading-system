...
trail_bricks   = input.float(2.0, "  Trail Distance (bricks)", minval=0.25, maxval=20.0, step=0.25, tooltip="Used ONLY by Fixed Bricks Behind - how far behind price the stop sits.", group=grp_mgmt)
trail_buf_bricks = input.float(0.0, "  Anchor Buffer (bricks)", minval=0.0, maxval=5.0, step=0.25, tooltip="Extra clearance beyond the Brick Grid or Swing anchor. 0 puts the stop exactly ON the swing low or brick edge, which is what following the candles means. Raise it only to allow for spread or a noisy instrument - every 0.25 here is a quarter of a brick of extra room given up.", group=grp_mgmt)
...
// Two different jobs, two different numbers. trail_dist is how far behind price
// the Fixed anchor sits. trail_buf is clearance beyond the Swing or Brick Grid
// anchor, and defaults to ZERO so the stop lands exactly on the swing low or
// brick edge. Sharing one input put the swing stop two full bricks below the
// candles, which is why it stopped looking like it was following them.
trail_dist = na(rk_box) ? 0.0 : trail_bricks * rk_box
trail_buf  = na(rk_box) ? 0.0 : trail_buf_bricks * rk_box
...
//
// Size and quantity are computed at TOP LEVEL, not inside the entry block, so
// the diagnostics panel can show them even on bars where no entry is attempted.
// A quantity that rounds to zero is a silent no-trade otherwise.
float dg_sd  = na(rk_box) ? na : stop_bricks * rk_box
float dg_qty = na
if use_risk_size and not na(dg_sd) and dg_sd > 0
    dg_qty := math.min(strategy.equity * risk_pct / 100.0 / dg_sd,
                       strategy.equity * max_equity_pct / 100.0 / close)

// The push test. Measured on the chart's own bars, which is also what fills
// use - this strategy is meant to be run on ordinary candles.
...
if (strategy.position_size == 0 or exited_now) and not cut_blocks and not na(rk_box) and rk_box > 0
    ...
    if go_long and confirm_long and (na(_qty) or _qty > 0)
        ...
        tp1_taken  := false
        peak_r     := 0.0
    else if go_short and confirm_short and (na(_qty) or _qty > 0)
...
plot(strategy.position_size != 0 and use_stop ? stop_px : na, "Stop",
     color=color.new(#F23645, 20), style=plot.style_linebr, linewidth=2)

// The raw anchor, before the ratchet. Plotting it makes an unmoving stop
// readable: if the anchor is climbing but the stop is flat, the ratchet is
// holding it because the anchor has not yet passed where the stop already is.
plot(show_diag and use_trail and strategy.position_size > 0 ? (trail_mode == "Brick Grid" ? rk_bot - trail_buf : trail_mode == "Fixed Bricks Behind" ? close - trail_dist : trail_lo_ref - trail_buf) : na,
     "Trail Anchor (long)", color=color.new(#B8860B, 45), style=plot.style_linebr, linewidth=1)
plot(show_diag and use_trail and strategy.position_size < 0 ? (trail_mode == "Brick Grid" ? rk_top + trail_buf : trail_mode == "Fixed Bricks Behind" ? close + trail_dist : trail_hi_ref + trail_buf) : na,
     "Trail Anchor (short)", color=color.new(#B8860B, 45), style=plot.style_linebr, linewidth=1)
...
if show_diag and barstate.islast
    ...
    _val  = array.from(
         ...
         not use_trail ? "off" : strategy.position_size == 0 ? "no position" : na(risk_unit) or risk_unit <= 0 ? "-" : ((strategy.position_size > 0 ? close - entry_px : entry_px - close) / risk_unit) < trail_start_r ? "inactive - needs " + str.tostring(trail_start_r, "#.#") + "R" : trail_mode + " @ " + str.tostring(stop_px, format.mintick))
    _pass = array.from(
         array.size(rk_fc) > 0,
